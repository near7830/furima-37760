class User < ApplicationRecord
  ...
  validates :first_name, presence: true ,format:{with: /\A[ぁ-んァ-ヶ一-龥々+ー]+\z/, message:'is invalid. Input full-width characters'}
  ...
  validates :password, format:{with: /\A(?=.*?[a-z])(?=.*?[\d])[a-z\d]+\z/i, message:'is invalid. Include both letters and numbers'}, length: {minimum: 6}
